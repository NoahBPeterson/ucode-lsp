...
| [117](../done/117-deep-expression-nesting-crash.md) | ✅ **FIXED 0.6.235** — deep expression nesting no longer crashes the server. A depth guard (MAX_ANALYSIS_DEPTH=1000) bails the recursive walkers (`visit`, `checkNode`) before the native stack overflows, and EVERY traversal entry point is contained (the escaping path was the post-catch flow-engine + inlay-hint walks), so any depth degrades to one honest "too deeply nested" Warning. Verified to 100k+ terms and across paren/array/ternary/unary nesting. | crash | med-high |
