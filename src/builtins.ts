export const builtinFunctions = new Map<string, string>([
    ...
    ['wildcard', 'Match the given subject string against the supplied wildcard (file glob) pattern. This function performs string matching and does not access the filesystem.\n\n**Parameters:**\n- `subject` - The subject string to match.\n- `pattern` - The wildcard pattern string (supports *, ?, [abc]).\n- `nocase` (boolean, optional) - If truthy, perform case-insensitive matching.\n\n**Returns:** `boolean` - True if pattern matches, false otherwise\n\n**Example:**\n```ucode\nwildcard(\"document.txt\", \"*.txt\")    // true\nwildcard(\"test1\", \"test?\")           // true\nwildcard(\"5files\", \"[0-9]*\")         // true\nwildcard(\"FILE.TXT\", \"*.txt\", true)  // true (case-insensitive)\n```'],
