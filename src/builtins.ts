export const builtinFunctions = new Map<string, string>([
    ...
    ['chr', 'Build a string from byte values, one byte per argument.\n\n**Parameters:**\n- `...codes` (integer) - Byte values; each is clamped to 0-255 (negative becomes 0, larger becomes 255), doubles truncate, and non-numeric values coerce to 0\n\n**Returns:** `string` - One byte per argument (never null; no arguments gives "")\n\n**Examples:**\n```ucode\nchr(97);        // "a"\nchr(72, 73);    // "HI"\nchr(-5);        // "\\0" (clamped)\nchr(300);       // "\\xff" (clamped)\n```'],
    ['ord', 'Get the byte value at a position in a string.\n\n**Parameters:**\n- `string` - The input string\n- `offset` (integer) *optional* - Byte position to read (default: 0). Negative counts from the end, like substr()\n\n**Returns:** `integer` - The byte value (0-255), or null if the input is not a string or the offset is out of range\n\n**Examples:**\n```ucode\nord("abc");      // 97\nord("abc", 1);   // 98\nord("abc", -1);  // 99\nord("abc", 9);   // null (out of range)\n```'],
    ...
    ['include', 'Include and execute a ucode file.\n\n**Parameters:**\n- `path` (string) - Path to file\n- `scope` (object, optional) - Variables to make available in included file\n\n**Returns:** `*` - Result of included file'],
    ['json', 'Parse JSON text into a ucode value. Accepts **only** a JSON string, or an object/resource with a `read()` method that streams one; every other argument raises a runtime error rather than returning null. `json()` cannot produce JSON — `sprintf("%J", value)` does that.\n\n**Parameters:**\n- `value` - JSON **text** to parse (a string), *or* a readable source: an object or resource with a callable `read(len)` method, which is streamed in chunks and parsed (e.g. the handle from `fs.open()` / `fs.popen()`)\n\n**Returns:** `*` - The parsed value (object, array, string, number, boolean, or null)\n\n**Can throw:** yes, on every failure — it never returns null:\n- a plain object or array *without* a `read()` method → `Input object does not implement read() method`\n- any other non-string value (number, boolean, null) → `Passed value is neither a string nor an object`\n- malformed JSON text → a syntax error\n\nGuard it with `try`/`catch`.\n\n**Example:**\n```ucode\nlet cfg = json(\'{"enabled": true}\');   // parse text\nlet fh = open("/etc/board.json", "r");\nlet board = json(fh);                  // stream + parse a file\nlet text = sprintf("%J", cfg);           // serialize (NOT json(cfg))\n```'],
    ...
    ['min', 'Find the minimum value from the arguments.\n\n**Parameters:**\n- `...values` - Values to compare\n\n**Returns:** `any` - The smallest value. The comparison works on any comparable values (not just numbers), so the result has the type of the chosen argument (e.g. `min("a","b")` → `"a"`).'],
    ['max', 'Find the maximum value from the arguments.\n\n**Parameters:**\n- `...values` - Values to compare\n\n**Returns:** `any` - The largest value. The comparison works on any comparable values (not just numbers), so the result has the type of the chosen argument (e.g. `max("a","b")` → `"b"`).'],
    ...
    ['loadstring', 'Compile uCode from a string into a callable function.\n\n**Parameters:**\n- `code` - String containing uCode source code\n- `options` *(optional)* - ParseConfig object: `raw_mode`, `strict_declarations`, `lstrip_blocks`, `trim_blocks`, `module_search_path`, `force_dynlink_list`\n\n**Returns:** `function | null` - Compiled function (call it to run the code), or `null` on error\n\n**Example:**\n```ucode\nloadstring("return 42")()                 // 42\nloadstring("1 + 1", { raw_mode: true })() // 2\n```'],
    ['loadfile', 'Compile a uCode file into a callable function.\n\n**Parameters:**\n- `path` - Path to uCode file\n- `options` *(optional)* - ParseConfig object: `raw_mode`, `strict_declarations`, `lstrip_blocks`, `trim_blocks`, `module_search_path`, `force_dynlink_list`\n\n**Returns:** `function | null` - Compiled function (call it to run the file), or `null` on error\n\n**Example:**\n```ucode\nloadfile("script.uc")()\nloadfile("tmpl.uc", { raw_mode: true })()\n```'],
    ...
    ['trace', 'Set the VM execution trace level.\n\n**Parameters:**\n- `level` (integer): New trace level to activate (0 disables tracing)\n\n**Returns:** `integer` - The previous trace level (0-255). Raises an exception if the argument is not an integer.\n\n**Example:**\n```ucode\nlet prev = trace(1);  // enable tracing, remember old level\n// ... code to trace ...\ntrace(prev);          // restore previous level\n```'],
    ['proto', 'Get or set a value\u2019s prototype \u2014 ucode\u2019s OOP mechanism (there is no `class`).\n\n**Parameters:**\n- `value`: the value whose prototype to read, or the ARRAY or OBJECT to attach one to\n- `prototype` (optional): the OBJECT to attach\n\n**Returns:** `object | null` \u2014 with one argument, `value`\u2019s current prototype (`null` if it has none). With two, it attaches `prototype` and **returns `value` itself**, unchanged in type: an array stays an `array`, keeping numeric indexing and `length()` while gaining the prototype\u2019s methods.\n\n**Throws** `Passed value is neither a prototype, resource or object` unless the target is an array or object **and** the prototype is an object. So `proto(x, null)` throws rather than detaching \u2014 there is no detach form \u2014 and a resource cannot be given a prototype (reading one works: a resource reports its resource type\u2019s shared prototype, which is how `fs.file` methods resolve).\n\nA later `proto(value, other)` **replaces** the prototype; it never merges. Member lookup walks the chain and stops at the first hit, so own members shadow the prototype\u2019s. In a method reached through the chain, `this` is the INSTANCE, not the prototype table.\n\n\u26a0\ufe0f Never make the chain cyclic (`proto(a, b); proto(b, a)`): reading a member that exists nowhere on it hangs the program forever.\n\n**Example:**\n```ucode\nconst wdev_proto = {\n\tget_name: function() { return this.name; },\n};\n\nlet wdev = proto({ name: "phy0" }, wdev_proto);\nwdev.get_name();               // "phy0" \u2014 `this` is wdev\nproto(wdev) === wdev_proto;    // true \u2014 one-argument form reads it back\n```'],
    ['render', 'Render a template, or invoke a function and capture its output. The behavior is selected by the first argument\'s type.\n\n**Form 1 — template (string first arg):**\n\nRender a template file with an optional scope (include-like).\n\n**Parameters:**\n- `path` (string): Template file path to render\n- `scope` (object, optional): Variables made available to the template\n\n**Returns:** `string | null` - Rendered output, or null on error\n\n**Example:**\n```ucode\nrender("/usr/share/templates/page.uc", { name: "Alice", count: 5 });\n```\n\n**Form 2 — function (function first arg, variadic):**\n\nCall the given function, forwarding all remaining arguments to it, and capture everything it prints into a string.\n\n**Parameters:**\n- `fn` (function): Function to invoke\n- `...args` - Arguments forwarded to `fn`\n\n**Returns:** `string | null` - The captured output of `fn`, or null on error\n\n**Example:**\n```ucode\nrender(render_page, request, response);  // calls render_page(request, response), returns its output\n```']
]);
